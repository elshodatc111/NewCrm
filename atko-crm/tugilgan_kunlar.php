...
<head>
  ...
  <script>
    <?php
      if(isset($_GET['send'])){echo "alert('Tug`ilgan kun tabriklari jo`natildi.');";}
    ?>
    </script>
</head>
    ...
    <section class="section contact">
        <div class="col-lg-12 col-12">
            <div class="info-box card p-2">
                <h5 class="card-title w-100 text-center pb-0 mb-1">Tug'ilgan kunlar yaqinlashmoqda</h5>
                <div class="table-responsive">
                    <table  class="table table-bordered text-center align-baseline table-striped" style="font-size:14px;" style='font-size:14px;' id="dataTable" width="100%" cellspacing="0">
                        <thead>
                        ...
                        <tbody>
                          <?php
                            $sql = "SELECT * FROM  users WHERE  DATE_ADD(TKun, INTERVAL YEAR(CURDATE())-YEAR(TKun) + IF(DAYOFYEAR(CURDATE()) > (TKun),1,0) YEAR) BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 5 DAY)";
                            $res = $conn->query($sql);
                            $i=1;
                            while ($row = $res->fetch()) {
                              echo "<tr>
                    ...
                    <div class="text-center" style="display:<?php if($Type==='mexmon'){echo 'none;';} ?>"><a href="./config/sms/tabrik.php" class="btn btn-warning" style="border-radius:0;">Tabrik jo'natish</a></div>
                  </div>
            </div>
        <div>
    </section>
